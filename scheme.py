...
def eval_all(expressions, env):
    ...
    list_ = expressions
    while list_.rest:
        scheme_eval(list_.first, env)
        list_ = list_.rest
    return scheme_eval(list_.first, env)# replace this with lines of your own code
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    line = expressions
    while line.rest:
        if is_false_primitive(scheme_eval(line.first, env)):
            return False
        line = line.rest
    return scheme_eval(line.first, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    line = expressions
    while line:
        if is_true_primitive(scheme_eval(line.first, env)):
            return scheme_eval(line.first, env)
        line = line.rest
    return False
    # END PROBLEM 12
